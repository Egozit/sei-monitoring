...
COS_BIN_NAME=$(which seid)
SEI_PATH=$(realpath .sei)
#SEI_CONFIG_PATH=${SEI_PATH}/config/config.toml
#IP_STR_NUM=$(($(grep -n -Rw $SEI_CONFIG_PATH -e 'RPC server to listen on' | cut -d : -f 1)+1))
#COS_PORT_RPC=$(sed -n ${IP_STR_NUM}p $SEI_CONFIG_PATH | cut -d : -f 3 | sed -e 's/"$//')
COS_PORT_RPC=$(seid config | grep node | cut -d '"' -f 4 | cut -d : -f 3)
#COS_VALOPER=$(seid keys show $COS_WALLET --bech val -a)
